...
public class Montreal {
	...
	public static MTLImpl mtlImpl;
	public static MTLImpl backupMtlImpl;
	...
	public static void main(String[] args) {
		...
		try {
			...
			Runnable ReplicaServer = () -> {
				receiveRequestFromReplicaManager(mtlImpl);
			};

			Runnable UDPServer = () -> {
				receiveRequestFromAnotherServer(mtlImpl);
			};
	...
	public static void receiveRequestFromReplicaManager(MTLImpl mtlobj) {
		String result = "";
		DatagramSocket ds = null;
		try {
			ds = new DatagramSocket(1001);
			byte[] receive = new byte[10000];
			System.out.println("UDP Server for Montreal is running on port : 1001");
			while (true) {
				...
				if (func.equals("addAppointment")) {
					...
					result = mtlobj.addAppointment(parameters.get(1), parameters.get(2), cap);
				} else if (func.equals("removeAppointment")) {
					result = mtlobj.removeAppointment(parameters.get(1), parameters.get(2));
				} else if (func.equals("listAppointmentAvailability")) {
					String[] ls = mtlobj.listAppointmentAvailability(parameters.get(2));
					...
					String[] ls = mtlobj.getAppointmentSchedule(parameters.get(0));
					result = String.join(",", ls).trim();
					...
					result = mtlobj.swapAppointment(parameters.get(0), parameters.get(1), parameters.get(2),
							parameters.get(4), parameters.get(5));
				} else if (func.equals("fault message")) {
					// result = mtlobj.handleFault();
					System.out.println("[INFO] ======== Faulty Replica Has Been Changed by Backup Replica =========");
					backupMtlImpl = new MTLImpl();
					mtlImpl = backupMtlImpl;
				}
	...
	public static void receiveRequestFromAnotherServer(MTLImpl mtlobj) {
		String result = "";
		DatagramSocket ds = null;
		try {
			...
			while (true) {
				...
				if (func.equals("list_appointment")) {
					...
					ls = mtlobj.listAppointmentAvailabilitySupport(parameters.get(0));
					result = String.join(",", ls).trim();
					...
					datastringarray = mtlobj.getAppointmentSchedule(parameters.get(0));
					data = Arrays.asList(datastringarray);
					result = String.join(",", data).trim();
				} else if (func.equals("cancel_appointment")) {
					if (parameters.size() == 2)
						result = mtlobj.cancelAppointment(parameters.get(0), parameters.get(1));
					if (parameters.size() == 3)
						result = mtlobj.cancelAppointment(parameters.get(0), parameters.get(1), parameters.get(2));
				} else if (func.equals("increase_appointment")) {
					mtlobj.increaseAppointmentCapacity(parameters.get(0), parameters.get(1));
				}
				// System.out.println(result);
				// //<----------------------------------------------------------------------------------------------testing
				result += "@";
...
class MTLImpl {
	...
	public String addAppointment(String appointmentID, String appointmentType, String[] capacity) {
		...
		synchronized (this) {
			...
			if (app_data.containsKey(appointmentType)) { // check same appointmentType exist or not
				temp = app_data.get(appointmentType);
				if (temp.containsKey(appointmentID)) { // check same appointmentID exist or not, if already exist that
														// can't add same appointmentID again
				...
				cap = caplist; // <-----------------------------------------------------
				temp.put(appointmentID, cap);
				app_data.put(appointmentType, temp);
				result = "Appointment added Successfully";
				status = "Success";
				// status = "Failed";
			}
			// Server Log
			try {
				serverLog("Admin", "Add appointment", appointmentID + "-" + appointmentType + "-" + cap, status,
						result);
			} catch (IOException e) {
				System.out.println(e);
			}
	...
	public String removeAppointment(String appointmentID, String appointmentType) {
		...
		if (app_data.containsKey(appointmentType)) {
			...
			if (temp.containsKey(appointmentID)) {
				...
				if (cap_data.size() > 1) { // there are patient reschedule the appointments for them than cancel the
											// appointment;
					String nextAppointment = nextAvailbaleAppointment(appointmentID, appointmentType);

					addAppointment(nextAppointment, appointmentType, cap_data.toArray(new String[cap_data.size()]));
					temp.remove(appointmentID, cap_data);
					result = "The " + appointmentID + " has been removed from and patient are rescheduled on "
							+ nextAppointment + " for " + appointmentType;
					status = "Success";
				} else {
					temp.remove(appointmentID, cap_data);
					result = "The " + appointmentID + " has been removed from " + appointmentType + " appointment.";
					status = "Success";
				}
			} else {
		...
		// Server Log
		try {
			serverLog("Admin", "Remove appointment", appointmentID + "-" + appointmentType, status, result);
		} catch (IOException e) {
			System.out.println(e);
		}
		return result + ";" + status;
	...
	public String[] listAppointmentAvailability(String appointmentType) {
		...
		result = this.listAppointmentAvailabilitySupport(appointmentType);
		String quebec_res = requestAnotherServer("list_appointment", params, que_port);
		String sherbrooke_res = requestAnotherServer("list_appointment", params, she_port);
		List<String> que_list = Stream.of(quebec_res.split(",", -1)).collect(Collectors.toList());
		List<String> she_list = Stream.of(sherbrooke_res.split(",", -1)).collect(Collectors.toList());
		result.addAll(que_list);
		result.addAll(she_list);
		// Server Log
		try {
			serverLog("Admin", "List appointment availability", appointmentType, "Success", String.join(", ", result));
		} catch (IOException e) {
			System.out.println(e);
		}
	...
	public String bookAppointment(String patientID, String appointmentID, String appointmentType) {
		...
		if ((appointmentID.substring(0, 3)).equals("MTL")) {
			if (canBookAppointmentToday(patientID, appointmentID, appointmentType)) {
				if (appointmentID.substring(0, 3).equals(patientID.substring(0, 3))) {
					flag = true;
				} else {
					flag = canbookthisweek(patientID, appointmentID, appointmentType);
				}
				if (flag) {
					if (app_data.containsKey(appointmentType)) { // check if AppointmnetType is exist or not
						temp = app_data.get(appointmentType);
						if (temp.containsKey(appointmentID)) { // Check is Appointment is available on that day
							cap = temp.get(appointmentID);
			...
			result = requestAnotherServer("book_appointment", params, que_port);
		...
		// Server Log
		try {
			serverLog("Patient " + patientID, "Book an appointment",
					patientID + " " + appointmentID + " " + appointmentType, status, result);
		} catch (IOException e) {
			System.out.println(e);
		}
		System.out.println(result);
	...
	public String[] getAppointmentSchedule(String patientID) {
		...
		for (Map.Entry mapElement : app_data.entrySet()) {
			...
			for (Map.Entry mapSubElement : temp.entrySet()) {
				...
				if (cap_data.contains(patientID)) {
					if (!(result.contains("Montreal Hospital : "))) {
						result.add("Montreal Hospital : ");
					}
		...
		if ((patientID.substring(0, 3)).equals("MTL")) {
			String quebec_res = requestAnotherServer("get_appointment", params, que_port);
			String sherbrooke_res = requestAnotherServer("get_appointment", params, she_port);
			// List<String> que_list = new
			// ArrayList<String>(Arrays.asList(quebec_res.split(",")));
			// List<String> she_list = new
			// ArrayList<String>(Arrays.asList(sherbrooke_res.split(",")));
			List<String> que_list = Stream.of(quebec_res.split(",", -1)).collect(Collectors.toList());
			List<String> she_list = Stream.of(sherbrooke_res.split(",", -1)).collect(Collectors.toList());
			result.addAll(que_list);
			result.addAll(she_list);
		...
		// Server Log
		try {
			serverLog("Patient " + patientID, "Get appointments schedule", patientID, "Success",
					String.join("-", result));
		} catch (IOException e) {
			System.out.println(e);
		}
	...
	public String cancelAppointment(String patientID, String appoinmentID) {
		...
		if ((appoinmentID.substring(0, 3)).equals("MTL")) {
			int flag = 0;
			for (Map.Entry mapElement : app_data.entrySet()) {
				String appType = (String) mapElement.getKey();
				Map<String, List<String>> temp = (Map<String, List<String>>) mapElement.getValue();
				...
				for (Map.Entry mapSubElement : temp.entrySet()) { // check AppointMentID is there or not
					String appID = (String) mapSubElement.getKey();
					if (appID.equals(appoinmentID)) { // if AppointMentID is there check the right patient is trying to
														// cancel the app...
						List<String> cap_data = (List<String>) mapSubElement.getValue();
		...
		} else if ((appoinmentID.substring(0, 3)).equals("QUE")) {
			result = requestAnotherServer("cancel_appointment", params, que_port);
	...
	public String swapAppointment(String patientID, String oldAppointmentID, String oldAppoinmentType,
		...
		boolean cancelOperation = cancelStatus
				.equals("the Appointment has been cancelled for the patient " + patientID);
		if (bookOperation && cancelOperation) { // both operation are successful
			...
			result = "the Appointment has been swapped for the patient " + patientID + " from old appointment id "
					+ oldAppointmentID + " to " + newAppointmentID;
			status = "Success";
			...
			result = "This operation cannot be perfomed by the patient " + patientID
					+ " because he/she neither booked the appointment nor canceled it";
			status = "Failed";
		}
		try {
			serverLog("Patient " + patientID, "Swap an appointment", patientID + " " + oldAppointmentID + " "
					+ oldAppoinmentType + " " + newAppointmentID + " " + newAppoinmentType, status, result);
		} catch (IOException e) {
			System.out.println(e);
		}
		return result + ";" + status;
	...
	public void increaseAppointmentCapacity(String appoinmentID, String appointmentType) {
		...
		if ((appoinmentID.substring(0, 3)).equals("MTL")) {
			...
			requestAnotherServer("increase_appointment", params, que_port);
		} else if ((appoinmentID.substring(0, 3)).equals("SHE")) {
	...
	public String cancelAppointment(String patientID, String appoinmentID, String appointmentType) {
		...
		if ((appoinmentID.substring(0, 3)).equals("MTL")) {
			...
			if (app_data.containsKey(appointmentType)) {
				...
				if (temp.containsKey(appoinmentID)) {
					List<String> cap_data = temp.get(appoinmentID);
					// check the patient are there or not
					if (cap_data.contains(patientID)) { // there are patient reschedule the appointments for them than
		...
		// Server Log
		try {
			serverLog("Patient " + patientID, "Cancel an appointment",
					patientID + " " + appoinmentID + " " + appointmentType, status, result);
		} catch (IOException e) {
			System.out.println(e);
		}
		return result + ";" + status;
	...
	public void serverLog(String user, String requestType, String params, String status, String result)
		...
		log_file = log_dir + "\\src\\ReplicaManager1\\Server_Logs\\montreal_logs.txt"; // log file path
		...
		pw.println(log_date + " " + user + " | " + requestType + " | " + params + " | " + status + " | " + result); // add
																													// the
																													// lof
																													// into
																													// logfile
		System.out.println("[INFO][LOG] " + log_date + " " + user + " | " + requestType + " | " + params + " | "
				+ status + " | " + result);
