...
class MTLImpl {
	...
	public String addAppointment(String appointmentID, String appointmentType, String[] capacity) {
		...
		synchronized(this) {
			...
			//Server Log
			try {
				serverLog("Admin","Add appointment",appointmentID+"-"+appointmentType+"-"+cap,status,result);
			}catch(IOException e){
				System.out.println(e);
			}
		...
		if(app_data.containsKey(appointmentType)) {
			...
			if(temp.containsKey(appointmentID)) {
				...
				if(cap_data.size()>1) { //there are patient reschedule the appointments for them than cancel the appointment;
					...
					result = "The "+appointmentID+" has been removed from and patient are rescheduled on "+nextAppointment+" for "+appointmentType;
					status = "Success";
				}else {
					temp.remove(appointmentID,cap_data);
	...
	public String bookAppointment(String patientID, String appointmentID, String appointmentType) {
		...
		//Server Log
		try {
			serverLog("Patient "+patientID,"Book an appointment",patientID+" "+appointmentID+" "+appointmentType,status,result);
		}catch(IOException e){
			System.out.println(e);
		}
