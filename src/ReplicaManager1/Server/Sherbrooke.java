...
public class Sherbrooke {
	public static SHEImpl sheImpl;
	public static SHEImpl backupSheImpl;
	...
	public static void main(String[] args) {
		...
		try {
			...
			Runnable ReplicaServer = () -> {
				receiveRequestFromReplicaManager(sheImpl);
			};

			Runnable UDPServer = () -> {
				receiveRequestFromAnotherServer(sheImpl);
			};
	...
	public static void receiveRequestFromReplicaManager(SHEImpl sheobj) {
		String result = "";
		DatagramSocket ds = null;
		try {
			ds = new DatagramSocket(3003);
			byte[] receive = new byte[10000];
			System.out.println("UDP Server for Sherbrooke is running on port : 3003");
			while (true) {
				...
				if (func.equals("addAppointment")) {
					...
					result = sheobj.addAppointment(parameters.get(1), parameters.get(2), cap);
				} else if (func.equals("removeAppointment")) {
					result = sheobj.removeAppointment(parameters.get(1), parameters.get(2));
				} else if (func.equals("listAppointmentAvailability")) {
					String[] ls = sheobj.listAppointmentAvailability(parameters.get(2));
					result = String.join(",", ls).trim();
				} else if (func.equals("bookAppointment")) {
					result = sheobj.bookAppointment(parameters.get(0), parameters.get(1), parameters.get(2));
					...
					result = sheobj.swapAppointment(parameters.get(0), parameters.get(1), parameters.get(2),
					...
					backupSheImpl = new SHEImpl();
					sheImpl = backupSheImpl;
				}
	...
	public static void receiveRequestFromAnotherServer(SHEImpl sheobj) {
		String result = "";
		DatagramSocket ds = null;
		try {
			...
			while (true) {
				...
				if (func.equals("list_appointment")) {
					...
					ls = sheobj.listAppointmentAvailabilitySupport(parameters.get(0));
					result = String.join(",", ls).trim();
					...
					datastringarray = sheobj.getAppointmentSchedule(parameters.get(0));
					...
					sheobj.increaseAppointmentCapacity(parameters.get(0), parameters.get(1));
				}
				// System.out.println(result);
				// //<----------------------------------------------------------------------------------------------testing
				result += "@";
...
class SHEImpl {
			...
			if (temp.containsKey(appointmentID)) {
				...
				if (cap_data.size() > 1) { // there are patient reschedule the appointments for them than cancel the
					...
					result = "The " + appointmentID + " has been removed from and patient are rescheduled on "
							+ nextAppointment + " for " + appoinmentType;
					status = "Success";
				} else {
					temp.remove(appointmentID, cap_data);
	...
	public String bookAppointment(String patientID, String appoinmentID, String appointmentType) {
		...
		String status = "";
		boolean flag = true;
		if ((appoinmentID.substring(0, 3)).equals("SHE")) {
		...
		for (Map.Entry mapElement : app_data.entrySet()) {
			...
			for (Map.Entry mapSubElement : temp.entrySet()) {
				...
				if (cap_data.contains(patientID)) {
					if (!(result.contains("Sherbrooke Hospital : "))) {
						result.add("Sherbrooke Hospital : ");
					}
	...
	public void serverLog(String user, String requestType, String params, String status, String result)
		...
		log_file = log_dir + "\\src\\ReplicaManager1\\Server_Logs\\sherbrooke_logs.txt"; // log file path
